...
const pins = [
  ...
  ["extensions see message_update before pi's listeners, and the extension event carries the same message object pi is about to render", "@earendil-works/pi-coding-agent/dist/core/agent-session.js", [/await this\._emitExtensionEvent\(event\);\s*\/\/ Notify all listeners\s*this\._emit\(/, /else if \(event\.type === "message_update"\) \{\s*const extensionEvent = \{\s*type: "message_update",\s*message: event\.message,/]],
  ["the extension runner hands each handler the event itself, not a copy, so a handler's change to event.message reaches pi's listeners", "@earendil-works/pi-coding-agent/dist/core/extensions/runner.js", [/async emit\(event\) \{\s*const ctx = this\.createContext\(\);\s*let result;\s*for \(const ext of this\.extensions\) \{\s*const handlers = ext\.handlers\.get\(event\.type\);\s*if \(!handlers \|\| handlers\.length === 0\)\s*continue;\s*for \(const handler of handlers\) \{\s*try \{\s*const handlerResult = await handler\(event, ctx\);/]],
